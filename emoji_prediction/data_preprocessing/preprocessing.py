...
def keep_words_surrounding_emoji(
        row, num_of_words_before, num_of_words_after,
        index_to_word, word_to_embedding,
        embedding_shape, want_embeddings=False):
    # Get indices of nonzero elements in the second column

    padded_emojis = np.pad(
        row['sequence_emojis'], (num_of_words_before - 1, num_of_words_after),
    ...
    new_column1 = [[padded_words[j] for j in range(
...
def generate_dataframes(size_in_MB):
    ...
    df = parse_to_df(
        data_path=data_path, file_path=data_path / file_name,
        size_to_read=int(size_in_MB * 1024 ** 2))
    df.to_csv(data_path / 'train.csv')

    flattened_df = pd.DataFrame(
        {'word': df['sequence_words'].explode(),
         'emoji': df['sequence_emojis'].explode()})
    ...
    with open(embedding_path, 'r', encoding='utf-8') as f:
        ...
        # vectorized_word_to_glove = np.vectorize(
        # lambda x: word_to_embedding.get(
        # x, np.zeros(embedding.shape).tolist()))

    new_df = df.apply(lambda x: keep_words_surrounding_emoji(
        x, 2, 2, None,
        None, None, False), axis=1)
    ...
    embedded_df = df.apply(
        lambda x: keep_words_surrounding_emoji(
            x, 2, 2, index_to_word,
            word_to_embedding, embedding.shape, True),
        axis=1).apply(
        lambda col: col.explode() if col.dtype == 'O' else col)
    embedded_df = embedded_df.dropna(subset=['words'])

    # sum_first_col = lambda row: pd.Series(
    #    {'words': np.sum(row['words'], axis=0), 'emoji': row['emoji']})
