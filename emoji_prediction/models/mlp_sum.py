...
class TqdmMetricsProgressBarCallback(Callback):
    ...
    def on_train_begin(self, logs=None):
        self.progress_bar = tqdm(
            total=self.total_epochs, unit='epoch',
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        self.progress_bar.set_postfix_str(
            f"Loss: {loss:.2f}, "
            f"Accuracy: {acc:.2f}%, F1 Score: {f1_score:.2f}, "
            f"Val Loss: {self.val_loss:.2f}, "
...
def train_fold(fold_number, fold_data, results_dict):
    ...
    # tqdm_callback = TqdmMetricsProgressBarCallback(
    # num_epochs, validation_data=(X_test, y_test),
    #                                               eval_interval=10)
    ...
    if parallel:
        with Manager() as manager:
            ...
            for i, (train_idxs, test_idxs) in enumerate(
                    kfold.split(X, np.argmax(y, axis=1))):
                fold_data = {
        ...
        for i, (train_idxs, test_idxs) in enumerate(
            ...
            fold_data = {
                ...
                # assuming num_gpus is the number of GPUs you have
                'train_idxs': train_idxs,
                'test_idxs': test_idxs,
                'X': X,
                'y': y,
